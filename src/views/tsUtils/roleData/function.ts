...
export const initialData = (role: ListObj) => {
  ...
  for (const code in RoleNumberAttribute) {
    let tmp: listObj;
    if (INTAttr.includes(RoleNumberAttribute[code])) {
      tmp = {
        code: code,
      ...
      tmp = {
        ...
        type: attributeName[code],
        defaultValue: defaultData[RoleNumberAttribute[code]],
        occupation: occupationData[RoleNumberAttribute[code]],
        favorite: favoriteData[RoleNumberAttribute[code]],
        algorithm: algoIncreaseData[RoleNumberAttribute[code]],
        update: 0,
        final: rolefinalData.value[RoleNumberAttribute[code]],
        gua: 0
      }
  ...
  watch(() => algoData.value, (val) => {
    ...
    dockerList.value.forEach((val) => {
      val.algorithm = algoIn[RoleNumberAttribute[val.code]]
    })
  }, { deep: true })
...
const getIncrease = (defaultData: roleTotalAttribute, increaseArr: Array<AttributeAddObj>) => {
  ...
  for (const add of increaseArr) {
    if (add.increaseMethod == AddOrPerAdd.ADD) {
      ...
      if(INTAttr.includes(add.attribute)){
        IncreaseData[add.attribute] += Math.floor((add.value / 100) * defaultData[add.attribute])
      }else{
