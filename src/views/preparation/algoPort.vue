<template>
    ...
    <el-card class="outerArea" v-if="draw.openType == areaType.one" :body-style="{ padding: '0px'}">
        ...
        <el-card class = "single" v-if="draw.type == portType.ONE_SINGLE"
            :body-style="{ padding: '0px' }">
            <div style="font-size:6px"> {{ toChinese(draw.algo[0].main, "main", draw.algo[0].suit as unknown as
                    string)
            }}</div>
            <div style="font-size:6px"> {{ toChinese(draw.algo[0].assistant1, "assist", draw.algo[0].suit as unknown as string)
            }}</div>
        </el-card>
        <img class="image" :src="getImg('no')" />
    </el-card>
    <el-card class="outerArea" v-if="draw.openType == areaType.two" :body-style="{ padding: '0px'}">
        <el-card class = "single" v-if="draw.type == portType.ONE_SINGLE || draw.type == portType.TWO_SINGLE"
            :body-style="{ padding: '0px' }">
        ...
        <el-card class = "single" v-if="draw.type == portType.TWO_SINGLE" 
            :body-style="{ padding: '0px'}">
            <div style="font-size:6px"> {{ toChinese(draw.algo[1].main, "main", draw.algo[1].suit as unknown as string)
            }}</div>
            <div style="font-size:6px"> {{ toChinese(draw.algo[1].assistant1, "assist", draw.algo[1].suit as unknown as string)
            }}</div>
        </el-card>
        <el-card class="onesuit" v-if="draw.type == portType.ONE_SUIT" 
                :body-style="{ padding: '0px'}">
                <div style="font-size:6px"> {{ toChinese(draw.algo[0].suit, "suit") }}</div>
                <div style="font-size:6px"> {{ toChinese(draw.algo[0].main, "main", draw.algo[0].suit as unknown as string)}}</div>
                <div style="font-size:6px"> {{ toChinese(draw.algo[0].assistant1, "assist", draw.algo[0].suit as unknown as string)}}</div>
                <div style="font-size:6px"> {{ toChinese(draw.algo[0].assistant2, "assist", draw.algo[0].suit as unknown as string)}}</div>
            </el-card>
